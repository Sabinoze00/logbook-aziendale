...
type SortKey = 'cliente' | 'oreTotaliPeriodo' | 'oreFiltrate' | 'collaboratori' | 'costoTotale' | 'fatturatoTotale' | 'margine' | 'marginePercentuale'
type SortDirection = 'asc' | 'desc'

export function ClientSummaryTable({ data, isLoading }: ClientSummaryTableProps) {
  ...
  // CSV Export function
  const exportToCSV = () => {
    const headers = ['Cliente', 'Ore Totali nel Periodo', 'Ore Lavorate (Filtrate)', 'Collaboratori', 'Costo Totale', 'Fatturato Totale', 'Margine €', 'Margine %']
    const csvContent = [
      headers.join(','),
      ...sortedData.map(row => [
        `"${row.cliente}"`,
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute('download', `riepilogo_clienti_${new Date().toISOString().split('T')[0]}.csv`)
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Riepilogo Clienti</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <tbody>
              {sortedData.map((row, index) => (
                <tr key={index} className="border-b hover:bg-gray-50">
                  <td className="py-3 px-4 font-medium text-black">{row.cliente}</td>
